...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (savedSection) {
      setSection(savedSection); // Set the saved section as the initial state
    }
  }, []);

  // Save the current section to localStorage whenever it changes
  useEffect(() => {
    if (section) {
      ...
      {see && (
        <div className="w-full md:w-1/5 bg-[#ffffff97] h-auto md:h-[100vh] rounded-lg ml-0 shadow-lg p-8">
          <ul className='text-[1.2rem] mt-8 cursor-pointer'>
            <li className={`p-4 hover:bg-[#5EA0EE] hover:text-white text-left rounded-md`} onClick={() => { setSection('explore_project'); if (window.innerWidth < 768) setSee(false);} }>Explore Projects</li>
            <li className='p-4 hover:bg-[#5EA0EE] hover:text-white text-left rounded-md' onClick={() => { setSection('explore_user') ; if (window.innerWidth < 768) setSee(false);}}>Explore Users</li>
            <li className='p-4 hover:bg-[#5EA0EE] hover:text-white text-left rounded-md' onClick={() => { setSection('my_project') ; if (window.innerWidth < 768) setSee(false);}}>My Projects</li>
            <li className='p-4 hover:bg-[#5EA0EE] hover:text-white text-left rounded-md' onClick={() => { setSection('notification') ; if (window.innerWidth < 768) setSee(false);}}>Notifications</li>
            <li className='p-4 hover:bg-[#5EA0EE] hover:text-white text-left rounded-md' onClick={() => { setSection('more') ; if (window.innerWidth < 768) setSee(false);}}>More</li>
          </ul>
        </div>
      )}
      {/* SIDE BLUE DOT */}
